...
Specialized test class for PDFTextParser class.
"""
import unittest
import os
...
class MyTestCase(unittest.TestCase):
    ...
    def test_Constructor002(self):
        ...
        """
        try:
            tfp = PDFTextParser(self.pdf_file)
...
Collaborate with Network Engineering, Service Assurance, Field Operations teams to understand the problem and devise
possible solution
...
"""
    ...
    def __createPDFFile(cls, filename):
        ...
        if cls.is_tool("enscript") and cls.is_tool("ps2pdf"):
            print("Using enscript to create PDF.")
            cmdline = "".join(["enscript", " ", os.path.abspath(cls.text_file), " -B -o - | ps2pdf - ", filename])
            print(cmdline)
            os.system(cmdline)
        else:
